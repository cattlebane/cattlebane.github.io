 ...
 * #		Hulu | Pre_Fall_and_Fall_Campaign | RichMedia_Quiz | Staging | 1.0
 * #
 * ################################################################################################ */
trace( "----------------------------------------------------------------------------------------------------------------------------" );
trace( " Staging_970x250_text_payload.js[ Hulu | Pre_Fall_and_Fall_Campaign | RichMedia_Quiz | Staging | 970x250 | 1.0 ]" );
trace( "  " );
trace( "  " );
...
function AdData( parsedJsonData ){
	...
	for ( i = 0; i < maxEndframeImages; i++ ) {
		...
		if ( ftData[curLabel + 'Image'] )
			this.endframeImages.push(ImageManager.addToLoad( ftData[curLabel + 'Image'], adParams.adPath ));
	}
...
function FtData() {
	...
	this.quizHeader = myFT.instantAds.quiz_header || 'If You Know Your TV...';
	this.quizCTA = myFT.instantAds.quiz_cta || 'Make Fall Yours. Sign Up Now';
	...
	this.question1answerD = myFT.instantAds.question_1_answer_d || 'FRIENDS';
	...
	this.question3 = myFT.instantAds.question_3 || 'Then you should know this show';
	...
	this.question5answerA = myFT.instantAds.question_5_answer_a || 'DATING NAKED';
	this.question5answerB = myFT.instantAds.question_5_answer_b || 'MISTRESSES';
	this.question5answerC = myFT.instantAds.question_5_answer_c || 'AMERICAN NINJA WARRIOR';
	this.question5answerD = myFT.instantAds.question_5_answer_d || 'BACHELOR IN PARADISE';
	...
	this.resultText2Good = myFT.instantAds.result_text_2_good || 'Thank goodness we found you.';
	this.resultText2OK = myFT.instantAds.result_text_2_ok || 'But you\'ve got some catching up to do.';
	this.resultText2Bad = myFT.instantAds.result_text_2_bad || 'You got all of them right!';
	...
	this.endframeCTA2 = myFT.instantAds.endframe_cta_2 || 'Sign Up Now';
	...
	this.question5Image = this.validateImagePath( 
		myFT.instantAds.question_5_image || 'instantAssets/quiz_bachinparadise.jpg'
	);
	this.endframeImage1 = this.validateImagePath( 
		myFT.instantAds.endframe_image_1 || 'instantAssets/ftBlankGif.gif'
	);
	this.endframeImage2 = this.validateImagePath( 
		myFT.instantAds.endframe_image_2 || 'instantAssets/ftBlankGif.gif'
	);
	this.endframeImage3 = this.validateImagePath( 
		myFT.instantAds.endframe_image_3 || 'instantAssets/ftBlankGif.gif'
	);
	this.endframeImage4 = this.validateImagePath( 
		myFT.instantAds.endframe_image_4 || 'instantAssets/ftBlankGif.gif'
	);
	this.endframeImage5 = this.validateImagePath( 
		myFT.instantAds.endframe_image_5 || 'instantAssets/ftBlankGif.gif'
	);
...
var Intro = ( function() {
	...
	self.hide = function() {
		if (self.showing) {
			...
			TweenLite.fromTo( self.container, 0.5, { clip:'rect(0px ' + adParams.adWidth + 'px ' + adParams.adHeight + 'px 0px)' }, { delay:0.35, clip:'rect(0px ' + adParams.adWidth + 'px ' + adParams.adHeight + 'px ' + adParams.adWidth + 'px)', ease:Quad.easeInOut } );
		}
	}
...
var Quiz = ( function() {
	...
	self.build = function ( args ) {
		...
		var gradientLeft = quizWidth + self.timer.width;
		...
		self.questionMarker = new Markup.addTextfield({
			...
			target: self.container,
			css: {
				width: args.marker.width,
				height: args.marker.height,
				top: args.marker.top,
		...
		TweenLite.set( self.questionMarker.container, { x: -args.marker.width, clip:'rect(0px ' + args.marker.width + 'px ' + args.marker.height + 'px ' + args.marker.width + 'px)' } );
	...
	self.show = function() {
		...
		TweenLite.from( self.headerTF.container, animationTime, { x: fromX, ease:Quad.easeOut } );
		TweenLite.from( self.subHeaderTF.container, animationTime, { delay: 0.2, x: fromX, ease:Quad.easeOut } );
		TweenLite.from( self.quizCTA.container, animationTime, { delay: 0.4, x: fromX, ease:Quad.easeOut } );
		TweenLite.to( self.gradient, animationTime, { delay: animationTime, x: 0, clip:'rect(0px ' + self.gradient.width + 'px ' + self.gradient.height + 'px 0px)', ease:Quad.easeInOut } );
		TweenLite.to( self.questionMarker.container, animationTime, { delay: animationTime, x: 0, clip:'rect(0px ' + Styles.getCss( self.questionMarker.container, 'width' ) + 'px ' + Styles.getCss( self.questionMarker.container, 'height' ) + 'px 0px)', ease:Quad.easeInOut } );

		TweenLite.delayedCall( quizBeginDelay, nextQuestion );

		TweenLite.delayedCall( (adData.questionTime / 2) + quizBeginDelay + 1, pause );
	}
	...
	function onAnswered(isCorrect) {
		trace('Quiz.onAnswered() isCorrect:' + isCorrect);
	...
	function nextQuestion() {
		if ( self.questionsQueue.length > 0 ) {
			...
			TweenLite.delayedCall( 0.2, self.callback );
	}
...
var ActionBTN = ( function() {
	...
	self.build = function ( args ) {
		...
		Align.move( Align.CENTER, copyTF.textfield );
	...
	self.simpleShow = function() {
		...
		TweenLite.delayedCall( 0.2, registerEvents );
	}
	...
	self.hide = function(quickHide) {
		...
		TweenLite.killDelayedCallsTo( registerEvents );
...
var QuizQuestion = ( function() {
	...
	self.build = function ( args ) {
		...
		for (var i = 0; i < 4; i++) {
			...
			questionData = adData.questions[self.id - 1];
	...
	self.timerAnimation = function() {
		self.imageAnimation = TweenLite.fromTo( self.image, adData.questionTime, { scale: self.startScale }, { scale: 1, ease:Quad.easeInOut, onComplete: onWrongAnswer } );
	}
	...
	self.hide = function() {
		TweenLite.to( self.container, 1, { opacity: 0, ease:Quad.easeInOut } );
	}
	...
	function showAnswer() {
		...
		TweenLite.fromTo( self.answerCORRECT.container, 0.5, { opacity: 1 }, { opacity: 0, ease:Quad.easeInOut } );

		TweenLite.delayedCall( 2, playVideo );
	}

	function playVideo() {
		TweenLite.delayedCall( 2, self.callback );	// temporary until we have a video player
	}
...
var QuizResultMarker = ( function() {
	...
	self.show = function() {
		for (var i = 0; i < self.answers.length; i++)
			self.answers[i].show( 0.5 + (0.2 * i) );
		TweenLite.to( self.imageContainer, 1, { opacity: 1, ease:Quad.easeIn } );
...
var Control = new function() {
	...
	this.postMarkup = function() {
		...
		Gesture.addEventListener( adData.elements.startBTN, GestureEvent.CLICK, Control.skipIntro);
	...
	this.skipIntro = function() {
		trace('Control.skipIntro()');
		adData.elements.intro.btn.onHitRollOver();
		Gesture.removeEventListener( adData.elements.startBTN, GestureEvent.CLICK, Control.skipIntro);

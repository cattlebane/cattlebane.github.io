...
var AdManager = new function() {
	...
	/** Method: init() 
		...
		This passes the settings object from parent scope and initialized the class. Expected settings include:

		jsonMapUrl 			- the url on which to request a map.json
		includeJsonDataByKey	- used to override the standard select-json-by-latest-non-expired method, and return
	...
	self.isPreviewLocation = function() {
		...
		if( href.match( /^file/ ) || 
			...
			href.match( /[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+:8000/ )
		) return true;
	...
	self.loadJsonMapComplete = function( target ) {
		...
		else if ( global.adParams.environmentId == 'espn' ) {
	...
	self.includeAdDataByKey = function( adArray, type ) {
		...
		trace( '              Only including ad data containing key: "' + self.adManagerSettings.includeJsonDataByKey.key + '" with values "' + self.adManagerSettings.includeJsonDataByKey.values.join(' or ' ) + '"' );
		...
		for( var i = 0; i < ads.length; i++ ) {
			for( var j = 0; j < self.adManagerSettings.includeJsonDataByKey.values.length; j++ ) {
				if( ads[ i ][ self.adManagerSettings.includeJsonDataByKey.key ] === self.adManagerSettings.includeJsonDataByKey.values[ j ] ) {
					adsWithKey.push( ads[ i ] );
				}
			}
		}
		if( adsWithKey.length > 0 ) {
			trace( '               ads with matched keys were found' );
			return adsWithKey;
		}
		else {

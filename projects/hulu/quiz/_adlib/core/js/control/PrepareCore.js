/** ----------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	Description:
		...
		[ DEPRECATED!! 6-6-2016, "PrepareCore_new.js" will be used until Node AdApp has propagated.]
...
var PrepareCore = new function() {
	...
	// init
	function init( _completeCallback ) {
		...
		// date settings
		if( adParams.dateSettings )
			prepareDateUtils( adParams.dateSettings );

		// json map
		if( adParams.adManager ) 
			prepareAdManager( adParams.adManager );

		// analytics
		if( adParams.analytics ) 
			prepareAnalytics( adParams.analytics );	
	...
	// optional, prepare date utils
	function prepareDateUtils( _settings ) {
		trace( 'PrepareCore.prepareDateUtils()' );
		DateUtils.initDateUtils( _settings );
	}




	// optional, prepare ad manager
	function prepareAdManager( _settings ) {
		trace( 'PrepareCore.prepareAdManager()' );
		async.wait();
		AdManager.init( _settings );
		AdManager.prepareJson( loadJsonComplete );
	}
	...
	// optional, prepare analytics
	function prepareAnalytics( _settings ) {
		trace( 'PrepareCore.prepareAnalytics()' );
		Analytics.init( _settings );
	}

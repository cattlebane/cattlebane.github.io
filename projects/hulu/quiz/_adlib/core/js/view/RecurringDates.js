	...
	Import:
		(start code)
			...
			var includes = function() { return [
				adParams.corePath + "js/view/RecurringDates.js",
				adParams.adPath + "build.js",
			]};
		(end code)	

	Implement Event Model:
		For an event that is every Friday at 6pm for one hour, the model would look like
		...
		After defining your event model, you must instantiate this class and submit it to <RecurringDates>.init(); This
		...
		(start code)
			...
			recurringEventsManager.init( dateModel ); // optionally, as a 2nd argument, specify a timezone-designation, like DateUtils.TZ_EST
		(end code)
		
		Now you can access the "next start date" and "next end date" methods. Use those returned Date objects, along with the standard <DateUtils> methods, to 
...
var RecurringDates = function() {
	...
	/** Method: init()
		...
		_dateModel 			- see the header-docs for a detailed example of what this object looks like
		_tzDesignation 		- Optional. Default is <DateUtils>.TZ_LOCAL. */
	...
	/** Method: getNextStartDate()
		...
		_name 				- corresponds to the name property in submitted date-model, see the header-docs for an 
								example of what such an object would look like */
